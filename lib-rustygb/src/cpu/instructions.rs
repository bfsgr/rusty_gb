...
impl PartialEq for Instruction {
    fn eq(&self, other: &Self) -> bool {
        ...
        unsafe {
            ...
            let a: fn([u8; 2], *mut Registers, *mut Bus) =
                transmute(self.function as fn([u8; 2], &mut Registers, &mut Bus) -> u8);
            let b: fn([u8; 2], *mut Registers, *mut Bus) =
                transmute(other.function as fn([u8; 2], &mut Registers, &mut Bus) -> u8);
...
impl Instruction {
    ...
    pub fn DAA(_operands: [u8; 2], registers: &mut Registers, _mem: &mut Bus) -> u8 {
        ...
        let mut adjust: u8 = if registers.test_flag(CARRY_FLAG) {
            0x60
        } else {
            0x00
        };

        if registers.test_flag(HALFCARRY_FLAG) {
            adjust |= 0x06;
        };
